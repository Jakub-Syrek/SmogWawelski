...
public class GtfsShapeService
{
    ...
    private const int    StepPts    = 3;

    /// <param name="cacheDir">Katalog cache. Null = LocalApplicationData.</param>
    public GtfsShapeService(string? cacheDir = null)
    {
        _cacheDir = cacheDir
    ...
    internal static int      IdxPublic(string[] hdr, string name) => Idx(hdr, name);
    internal static string[] SplitCsvPublic(string line)          => SplitCsv(line);
